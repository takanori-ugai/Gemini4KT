...
class ToolBuilderTest {
    ...
    fun testToolBuilderWithNestedGoogleSearchConfiguration() {
        ...
        val fileSearch = checkNotNull(decoded.fileSearch)
        val googleSearchRetrieval = checkNotNull(decoded.googleSearchRetrieval)
        val computerUse = checkNotNull(decoded.computerUse)
        val mcpServer = checkNotNull(decoded.mcpServers).first()
        val googleMaps = checkNotNull(decoded.googleMaps)
        val searchTypes = checkNotNull(googleSearch.searchTypes)

        assertEquals("author=alice", fileSearch.metadataFilter)
        assertEquals("store-a", fileSearch.fileSearchStoreNames?.first())
        assertEquals(DynamicRetrievalMode.MODE_DYNAMIC, googleSearchRetrieval.dynamicRetrievalConfig?.mode)
        ...
        assertEquals("mcp-server", mcpServer.name)
        assertEquals(true, googleMaps.enableWidget)
        ...
        assertNotNull(searchTypes.webSearch)
        assertNotNull(searchTypes.imageSearch)
    }
