...
class GeminiTest {
    ...
    @Test
    fun `streamGenerateContent yields responses on success`() = runTest {
    ...
    @Test
    fun `getContent with inputJson returns content on success`() = runTest {
    ...
    @Test
    fun `getContent without inputJson returns content on success`() = runTest {
        val response = """{"key":"value"}"""
        gemini = createGemini { request ->
    ...
    @Test
    fun `getContent returns empty json on error`() = runTest {
        gemini = createGemini {
            respond(
    ...
    @Test
    fun `deleteContent succeeds with 200 response`() = runTest {
        gemini = createGemini { request ->
    ...
    @Test
    fun `deleteContent handles error response`() = runTest {
         gemini = createGemini {
             respond(
                 content = "Error",
                 status = HttpStatusCode.BadRequest
    ...
    @Test
    fun `generateContent calls getContent with correct parameters`() = runTest {
    ...
    @Test
    fun `createCachedContent calls getContent with correct parameters`() = runTest {
    ...
    @Test
    fun `listCachedContent calls getContent with correct parameters`() = runTest {
    ...
    @Test
    fun `getCachedContent calls getContent with correct parameters`() = runTest {
    ...
    @Test
    fun `deleteCachedContent calls deleteContent with correct parameters`() = runTest {
        val name = "cachedContent-123"
        gemini = createGemini { request ->
            assertEquals("$baseUrl/$name", request.url.toString())
            assertEquals(HttpMethod.Delete, request.method)
    ...
    @Test
    fun `countTokens calls getContent with correct parameters`() = runTest {
    ...
    @Test
    fun `batchEmbedContents calls getContent with correct parameters`() = runTest {
    ...
    @Test
    fun `embedContent calls getContent with correct parameters`() = runTest {
    ...
    @Test
    fun `getModels calls getContent with correct parameters`() = runTest {
    ...
    @Test
    fun `uploadFile calls fileUploadProvider with correct parameters`() = runTest {
